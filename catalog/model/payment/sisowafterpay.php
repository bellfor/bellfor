<?php
class ModelPaymentSisowafterpay extends Model {
	public function getMethod($address, $total) {
		$this->load->language('payment/sisowafterpay');
		...
		if ($this->config->get('sisowafterpay_total') > 0 && $this->config->get('sisowafterpay_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('sisowafterpay_geo_zone_id')) {
			$status = true;
