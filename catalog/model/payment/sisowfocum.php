<?php
class ModelPaymentSisowfocum extends Model {
	public function getMethod($address, $total) {
		$this->load->language('payment/sisowfocum');
		...
		if ($this->config->get('sisowfocum_total') > 0 && $this->config->get('sisowfocum_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('sisowfocum_geo_zone_id')) {
			$status = true;
