...
class ModelPaymentAmazonLoginPay extends Model {
	...
	public function getAdditionalCharges($order_id) {
		return $this->db->query("SELECT `title`, `order_total_id`, `value` FROM `" . DB_PREFIX . "order_total` WHERE `order_id` = " . (int)$order_id)->rows;
	}

	public function getOrderTotal($order_id) {
		return $this->db->query("SELECT `title`, `order_total_id`, `value` FROM `" . DB_PREFIX . "order_total` WHERE `order_id` = " . (int)$order_id ." AND `code` ='total'")->row['value'];
	}	
