<?php
class ModelPaymentSisowbelfius extends Model {
	public function getMethod($address, $total) {
		$this->load->language('payment/sisowbelfius');
		...
		if ($this->config->get('sisowbelfius_total') > 0 && $this->config->get('sisowbelfius_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('sisowbelfius_geo_zone_id')) {
			$status = true;
