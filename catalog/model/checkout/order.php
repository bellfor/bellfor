...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		//Modification: removal of the auxiliary array, if it is not deleted, it will be recorded in the database as an empty line, due to which all orders will be assigned the status of "0"!!!
        if ($data['totals']['products_total_minus_discount']){
            unset($data['totals']['products_total_minus_discount']);
        }
        /*---------------------------------------------------------*/
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
            ...
            $temp_code = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE `order_id` = '" . (int)$order_id . "' AND `code` = 'coupon'");
            // If current order status is not processing or complete but new status is processing or complete then commence completing the order
//			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
            if ((!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) || $temp_code->rows[0]['code'] == 'coupon') {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
			  ...
			  //Selecting a delivery method between DPD and DHL for German customers
				if(!empty($this->session->data['rumun'])) {
				//$data['text_shipping_method']
				//$text .= 'popopo ' . $this->session->data['rumun'] . "\n";
				$data['shipping_method'] = $data['shipping_method'] ." (<strong>".$this->session->data['rumun']."</strong>)";

				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET `shipping_method` = CONCAT(`shipping_method`, ' (" . $this->db->escape($this->session->data['rumun']) . ")') WHERE order_id = '" . (int)$order_id . "'");

				$this->session->data['rumun']=null;
				}
				if(isset($this->session->data['rumun'])) {
					unset($this->session->data['rumun']);
				}
