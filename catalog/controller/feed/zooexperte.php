...
class ControllerFeedZooexperte extends Controller
{
    ...
    {
        if ($this->config->get('zooexperte_status')) {
            //  Start TIMER
            ...
            $isDefaultLang=true;
            $file_append="";
            $file_only=$this->config->get('zooexperte_file');
            $zooexperte_description=$this->config->get('zooexperte_description');
            $zooexperte_description_html=$this->config->get('zooexperte_description_html');
            $zooexperte_feed_id1=$this->config->get('zooexperte_feed_id1');
            $zooexperte_use_taxes=$this->config->get('zooexperte_use_taxes');
            $product_url="";
            if (isset($_GET['store'])) {
            ...
            if (isset($_GET['lang'])) {
                $lang=$this->model_feed_zooexperte->getLangID($_GET['lang']);
            ...
            if ($file_only) {
                $filetitle='feeds/zooexperte'.$file_append.'.xml';
            ...
            //$products = $this->model_feed_zooexperte->getProducts($lang,$store);
            $zooexperte_availability=$this->config->get('zooexperte_availability');
            $use_tax=true;
            if ($zooexperte_use_taxes==2) {
                $use_tax=false;
            }
            $tax_rate='';
            $tax_rateArray=$this->model_feed_zooexperte->getTax();
            ...
            $attribute_id=$this->config->get('zooexperte_attribute');
            $attribute_id_type=$this->config->get('zooexperte_attribute_type');
            $shippingArray=$this->model_feed_zooexperte->getShipping();
            $shippingFlat=$this->config->get('zooexperte_shipping_flat');
            $shipping="";
            ...
            $is_apparel=0;
            $apparel_option_id = $this->config->get('zooexperte_option');
            $limit=1000;
            $start=0;
            $data_limit = array(
                "start" => $start,
            ...
            $products = $this->model_feed_zooexperte->getProducts($lang, $store, $data_limit);
            while (count($products)>0) {
                foreach ($products as $product) {
                    if ($product['quantity']==0 && $zooexperte_availability=="skip products" || in_array($product['product_id'], $black_product_id)) {
                        continue;
                    }
...
", " ", str_replace("\t", " ", str_replace("\n", " ", str_replace("\r", " ", str_replace("\r\n", " ", htmlspecialchars($this->strip_html_tags(htmlspecialchars_decode($product_details, ENT_COMPAT)), ENT_COMPAT, 'UTF-8'))))));
                    ...
                    foreach ($categories as $category) {
                        ...
                        if ($path) {
                            ...
                            foreach (explode('_', $path) as $path_id) {
                                $category_info = $this->model_feed_zooexperte->getCategory($path_id, $lang, $store);
                                $count++;
                                if ($category_info) {
                                    if (!$string) {
                    ...
                    $category_id_google = $this->model_feed_zooexperte->getTaxonomy($category_id);
                    if (isset($category_id_google['taxonomy_id'])) {
                        $is_apparel = $this->model_feed_zooexperte->isApparel($category_id_google['taxonomy_id']);
                        $item .= '<g:google_product_category>' . $category_id_google['taxonomy_id'] .'</g:google_product_category>';
                    }
                    $apparel_data = $this->model_feed_zooexperte->getProductExtra($product['product_id'], $attribute_id, $lang);
                    if ($is_apparel) {
                        $item .= '<g:item_group_id>' . $product['product_id'] . '</g:item_group_id>';
                        if ($zooexperte_feed_id1=='model') {
                    ...
                    if ($attribute_id_type=='-1') {
                        ...
                        $product_type = $this->model_feed_zooexperte->getProductExtraType($product['product_id'], $attribute_id_type, $lang);
                        if ($product_type=='') {
                            $item .= $cat_string;
                        } else {
                    ...
                    if ($product['quantity']==0) {
                        $item .= '<g:availability>' . $zooexperte_availability . '</g:availability>';
                    } else {
                    ...
                    if ($is_apparel) {
                        $options=$this->model_feed_zooexperte->getProductOptions($product['product_id'], $apparel_option_id, $lang);
                        $itemTMP=$item;
                        if (count($options)) {
                            $item='';
                        }
    ...
    {
        $category_info = $this->model_feed_zooexperte->getCategory($parent_id, $lang, $store);
    ...
    {
        while (ControllerFeedZooexperte::startsWith($string, $remove)) {
            $string = substr($string, strlen($remove));
        }
        while (ControllerFeedZooexperte::endsWith($string, $remove)) {
