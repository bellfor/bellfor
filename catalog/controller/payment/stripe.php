...
//        die;
        ...
        $amount = str_replace(".", "", $_SESSION['default']['ADtotal']);

//        $event = json_decode($_REQUEST);
...
//        }
        ...
        if($result){
            ...
            header('Location: '.$this->url->link('checkout/onepagecheckout',array('ideal_error'=>'Payment is not ready')));
        }
			...
			if (empty($json['success'])) {
                ...
                if($type == "EPS"){
                    $source = \Stripe\Source::create(array(
                        ...
                        'redirect' => array('return_url' => "https://www.bellfor.info/index.php?route=payment/stripe/webhook"),
                    ));

                    $json['success'] =  $source->redirect->url;
                }elseif($type == "IDEAL"){
                    $source = \Stripe\Source::create(array(
                        'type' => 'ideal',
                        'amount' => (int)number_format($total * 100, 0, '.', ''),
