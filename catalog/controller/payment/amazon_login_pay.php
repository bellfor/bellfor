...
class ControllerPaymentAmazonLoginPay extends Controller {
	...
	public function confirm() {
		...
		if ($this->config->get('config_checkout_id')) {
			...
			if ($information_info) {
				...
				$data['error_agree'] = $data['text_agree'] = '';
			}
		} else {
			$data['error_agree'] = $data['text_agree'] = '';
		}
		...
		if ($this->config->get('config_cancel_id')) {
			...
			$cancel_info = $this->model_catalog_information->getInformation($this->config->get('config_cancel_id'));

			if ($cancel_info) {
				$data['text_cancel'] = sprintf($this->language->get('text_cancel'), $this->url->link('information/information', 'information_id=' . $this->config->get('config_cancel_id'), 'SSL'), $cancel_info['title'], $cancel_info['title']);
			} else {
				$data['text_cancel'] = '';
			}
		} else {
		...
		//end fix by oppo webiprog.com  05.12.2017
	...
	public function processOrder() {
		...
		$total = $this->model_payment_amazon_login_pay->getOrderTotal($order_info['order_id']);
		//print $total; die();
		if (!isset($this->session->data['lpa']['AmazonOrderReferenceId'])) {
			$this->failure($this->language->get('error_process_order'));
		}
