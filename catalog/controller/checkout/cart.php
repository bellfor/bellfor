	...
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			if ($this->cart->getProducts())
			{
				...
				$this->load->model('module/freeshippingbytotalprice');

				if ($this->customer->isLogged())
				{
					$freeshippings = $this->model_module_freeshippingbytotalprice->getFreeshippingCustomer($this->customer);
				}
				else
				{
					$freeshippings = $this->model_module_freeshippingbytotalprice->getFreeshippingGuest();
				}

				if (!empty($freeshippings) && is_array($freeshippings) && count ($freeshippings))
				{
					$freeshippings = min($freeshippings);

					if ($total_cart)
					{
						if ($total_cart < $freeshippings)
						{
							$this->load->language('module/freeshippingbytotalprice');

							$pricefreeshipping = $freeshippings - $total_cart ;
							$text_price = sprintf(sprintf($this->language->get('text_free_price'), $this->currency->format($pricefreeshipping)));

							$data['freeshippingbytotalprice'] = '<div id="pricefreeshipping" class="text-left"><div class="list-group bg-info"><span class="pricefreeshipping-list-group-item" style="background-color: #ffc70f;font-size:16px;color: #a94442;border: 1px solid #7E842E;display: block;padding: 10px 15px">'.$text_price.'</span></div></div>';
						}
					}
				}
			}
		...
		if ($product_info) {
			...
			if (!$json) {
                ...
                if (isset($related_products)) {
                    ...
                    $json['text_related_title'] = $this->language->get('text_related_title');
                }
                ...
                $json['count_item'] = '(' . $this->cart->countProducts() .  $this->language->get('count_item') . ')';
