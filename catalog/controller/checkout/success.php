...
class ControllerCheckoutSuccess extends Controller {
	public function index() {
		...
		if (isset($this->session->data['order_id'])) {
            ...
            $data['order_info'] = $this->model_checkout_order->getOrderInfo($this->session->data['order_id']);

            $data['order_info']['country_code'] = strtolower($this->config->get('config_geocode'));

            if ($this->config->get('config_store_id') == 3) {
                $data['order_info']['country'] = strtolower($this->config->get('config_geocode'));
                $data['order_info']['merchantId'] = 100476268;
            } else {
                $data['order_info']['country'] = 'de';
                $data['order_info']['merchantId'] = 100476118;
            }
