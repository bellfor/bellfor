...
class ModelPaymentStripe extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('stripe_total') > 0 && $this->config->get('stripe_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('stripe_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				'code'       => 'stripe',
				'title'      => ($this->config->get('stripe_test') ? $this->language->get('text_title_test') : $this->language->get('text_title')),
				'terms'      => '',
