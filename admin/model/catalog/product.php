...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', email_required = '" . (int)$data['email_required'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");

		$product_id = $this->db->getLastId();

        if (isset($data['prices'])) {
            foreach ($data['prices'] as $price_info => $price) {
//                $price_info = explode('-', $key);

//                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_price` SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$price_info[1] . "', price = '" . $price . "'");
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_price` SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$price_info . "', price = '" . $price . "'");
            }
        }
		...
		if (isset($data['product_discount'])) {
			foreach ($data['product_discount'] as $store_id => $store_products) {
                foreach ($store_products as $product_discount) {
				    $this->db->query("INSERT INTO " . DB_PREFIX . "product_discount SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id . "', customer_group_id = '" . (int)$product_discount['customer_group_id'] . "', quantity = '" . (int)$product_discount['quantity'] . "', priority = '" . (int)$product_discount['priority'] . "', price = '" . (float)$product_discount['price'] . "', date_start = '" . $this->db->escape($product_discount['date_start']) . "', date_end = '" . $this->db->escape($product_discount['date_end']) . "'");
			    }
			}
		}

		if (isset($data['product_special'])) {
			foreach ($data['product_special'] as $store_id => $store_products) {
                foreach ($store_products as $product_special) {
				    $this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', priority = '" . (int)$product_special['priority'] . "', price = '" . (float)$product_special['price'] . "', date_start = '" . $this->db->escape($product_special['date_start']) . "', date_end = '" . $this->db->escape($product_special['date_end']) . "'");
			    }
			}
		}
	...
	public function editProduct($product_id, $data) {
        ...
        foreach ($data['prices'] as $store_id => $price) {
            $this->db->query("DELETE FROM `" . DB_PREFIX . "product_price` WHERE product_id = '" . (int)$product_id . "' AND store_id = '" . (int)$store_id . "'");
        }

        if (isset($data['prices'])) {
            foreach ($data['prices'] as $store_id => $price) {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "product_price` SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id . "', price = '" . $price . "'");
            }
        }
...
//                $this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "'");
    ...
    public function getProductFeed($product_id) {
