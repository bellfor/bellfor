...
class ModelBlogReview extends Model {
	public function addReview($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "review_article SET author = '" . $this->db->escape($data['author']) . "', article_id = '" . (int)$data['article_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");

		$review_article_id = $this->db->getLastId();

		$this->cache->delete('article');

		$this->event->trigger('post.admin.review.add', $review_article_id);
	...
	public function editReview($review_article_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "review_article SET author = '" . $this->db->escape($data['author']) . "', article_id = '" . (int)$data['article_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE review_article_id = '" . (int)$review_article_id . "'");

		$this->cache->delete('article');

		$this->event->trigger('post.admin.review.edit', $review_article_id);
	}

	public function deleteReview($review_article_id) {
		$this->event->trigger('pre.admin.review.delete', $review_article_id);

		$this->db->query("DELETE FROM " . DB_PREFIX . "review_article WHERE review_article_id = '" . (int)$review_article_id . "'");

		$this->cache->delete('article');

		$this->event->trigger('post.admin.review.delete', $review_article_id);
	}
