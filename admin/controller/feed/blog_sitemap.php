...
class ControllerFeedBlogSitemap extends Controller {
	...
	public function index() {
		$this->load->language('feed/blog_sitemap');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('feed/blog_sitemap', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('feed/blog_sitemap', 'token=' . $this->session->data['token'], 'SSL');
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'feed/blog_sitemap')) {
