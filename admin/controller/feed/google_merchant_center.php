...
class ControllerFeedGoogleMerchantCenter extends Controller {
	...
	public function index() {
		...
		/**
		 * http://localhost/bellfor.info_opc/index.php?route=feed/google_merchant_center&lang=de&curr=EUR&store=0
		 */
		...
		$feed_array[] = $this->config->get('config_name').PHP_EOL.HTTP_CATALOG . 'index.php?route=feed/google_merchant_center&lang='.$this->config->get('config_language').'&curr='.$this->config->get('config_currency').'&store='.(int)$this->config->get('config_store_id');

		foreach($tupz as $ss =>$vf) {
         $feed_array[] = $vf['config_name'].PHP_EOL.$vf['ssl'] . 'index.php?route=feed/google_merchant_center&lang='.$vf['config_language'].'&curr='.$vf['config_currency'].'&store='.(int)$ss;
		}
