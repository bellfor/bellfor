...
class ControllerPaymentKlarnaInvoicePro extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			foreach ($this->request->post['klarna_invoice_pro'] as $klarna_invoice_pro) {
				if ($klarna_invoice_pro['status']) {
					$status = true;
			...
			$klarna_data = array(
				'klarna_invoice_pro_pclasses' => $this->pclasses,
				'klarna_invoice_pro_status'   => $status
			);

			$this->model_setting_setting->editSetting('klarna_invoice_pro', array_merge($this->request->post, $klarna_data));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/klarna_invoice_pro', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/klarna_invoice_pro', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['countries'][] = array(
			'name' => $this->language->get('text_austria'),
			'code' => 'AUT'
		);		

		if (isset($this->request->post['klarna_invoice_pro'])) {
			$data['klarna_invoice_pro'] = $this->request->post['klarna_invoice_pro'];
		} else {
			$data['klarna_invoice_pro'] = $this->config->get('klarna_invoice_pro');
		}
		...
		$file = DIR_LOGS . 'klarna_invoice_pro.log';
		...
		$data['clear'] = $this->url->link('payment/klarna_invoice_pro/clear', 'token=' . $this->session->data['token'], 'SSL');

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('payment/klarna_invoice_pro.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/klarna_invoice_pro')) {
	...
	public function clear() {
		...
		$this->response->redirect($this->url->link('payment/klarna_invoice_pro', 'token=' . $this->session->data['token'], 'SSL'));
	}
}