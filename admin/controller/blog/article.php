...
class ControllerBlogArticle extends Controller {
	...
	protected function getList() {
		...
		$data['sort_noindex'] = $this->url->link('blog/article', 'token=' . $this->session->data['token'] . '&sort=p.noindex' . $url, 'SSL');
	...
	protected function getForm() {
		...
		$data['entry_status'] = $this->language->get('entry_status');
		$data['entry_consultant'] = $this->language->get('entry_consultant');
		$data['entry_race'] = $this->language->get('entry_race');
		$data['select_race'] = $this->language->get('select_race');
		$data['entry_noindex'] = $this->language->get('entry_noindex');
		...
		$data['help_race'] = $this->language->get('help_race');
        ...
        if (isset($this->error['race_id'])) {
            $data['error_race'] = $this->error['race_id'];
        } else {
            $data['error_race'] = '';
        }
        ...
        if (isset($this->request->post['status_consultant'])) {
            $data['status_consultant'] = $this->request->post['status_consultant'];
        } elseif (!empty($article_info)) {
            $data['status_consultant'] = $article_info['status_consultant'];
        } else {
            $data['status_consultant'] = true;
        }
        ...
        $race_dogs = $this->model_blog_article->getRace();

		if (isset($race_dogs[0])){
            foreach ($race_dogs as $race_dog){
                $data['race_dogs'][] = array(
                    'race_id' => $race_dog['race_id'],
                    'race'    => $race_dog['race']
                );
                if (!empty($article_info['race_id']) && $article_info['race_id'] == $race_dog['race_id']){
                    $data['choose_race'] = array(
                        'race_id' => $race_dog['race_id'],
                        'race'    => $race_dog['race']
                    );
                }
	...
	protected function validateForm() {
        ...
        if ($this->request->post['status_consultant']){
            if (!$this->request->post['race_id']){
                $this->error['race_id'] = $this->language->get('error_race');
            }
