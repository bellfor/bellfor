...
class ControllerExtensionModuleAutoAddProducts extends Controller {
	private $type = 'module';
	...
	public function index() {
		...
		$data['settings'][] = array(
			'type'		=> 'tabs',
			'tabs'		=> array('extension_settings', 'triggers', 'product_groups', 'rule_sets', 'testing_mode'),
		);
		$data['settings'][] = array(
		...
		$data['settings'][] = array(
			'type'		=> 'html',
			'content'	=> '<div class="text-info text-center pad-bottom">' . $data['help_triggers'] . '</div>',
		);
		$data['settings'][] = array(
			'key'		=> 'triggers',
			'type'		=> 'heading',
		...
		$data['settings'][] = array(
			'key'		=> $table,
			'type'		=> 'table_start',
			'columns'	=> array('action', 'name', 'products_to_auto_add', 'rules'),
		);
		foreach ($this->getTableRowNumbers($data, $table, $sortby) as $num => $rules) {
			...
			$data['settings'][] = array(
				'key'		=> $prefix . 'auto_added_free',
				'type'		=> 'select',
				'options'	=> array(1 => $data['text_yes'], 0 => $data['text_no']),
				'default'	=> 0,
				'before'	=> '<br />' . $data['entry_auto_added_free'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'auto_added_removable',
				'type'		=> 'select',
				'options'	=> array(1 => $data['text_yes'], 0 => $data['text_no']),
				'default'	=> 0,
				'before'	=> $data['entry_auto_added_removable'],
			);
			$data['settings'][] = array(
				'key'		=> $prefix . 'auto_remove_products',
				'type'		=> 'select',
				'options'	=> array(1 => $data['text_yes'], 0 => $data['text_no']),
				'default'	=> 0,
				'before'	=> $data['entry_auto_remove_products'],
			);
			$data['settings'][] = array(
				'type'		=> 'column',
			);
